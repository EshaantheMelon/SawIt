...
Changed files include the the views in the src folder and all files in the public folder of asset_client. 

Help the good guys beat the middlemen using blockchain, stop the bad guys burning forests.
Disrupt the agricultural industry and save the planet in line with SDGs 9,10,11,12 and 13.

QR-Blockchain traceability tool cross referenced with data on potential plantations from satellite data-based game. 
Built using Google Maps API and Hyperledger Sawtooth as a Progressive Web App. 
Future developments include using the tagged images of potential plantations to train an online Machine learning model to make predictions on plantation locations. 
